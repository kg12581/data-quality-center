"""大数据连接示例（用例名见同名 yaml 里的 name 字段）：
规则见同目录的「1a572f6e-9144-44d1-92c1-ccebf4f02360.yaml」。

规则集里写的是连接名（conn: dw_hive），真正的 IP / 端口 / 账号密码 / jar 放在 .env：
    python 1a572f6e-9144-44d1-92c1-ccebf4f02360.py --env /opt/conf/prod.env
"""
...
ENV_FILE = None                                # None = 仓库根目录的 .env（服务器可写 /opt/conf/prod.env）
REPORT_FORMATS = ("json", "md", "html")
OUTPUT_DIR = None
EXTRA_VARS = {}                                # 例如按天跑：{"partition": "dt = '2026-09-16'"}
ONLY = []
SHOW_FAILED_ROWS = False
